	...
	public TMPnew() {
		addWindowListener(new WindowAdapter() {
			...
			public void windowActivated(WindowEvent arg0) {
				/*KeyStroke ctrlBKey = KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, InputEvent.CTRL_MASK);
				if(ctrlBKey != null){
					dispose();
				}*/
				CAM1 = Cam();
	...
	public JPanel Cam(){
		...
		if(dsi == null){
			...
			for(int  i = 0; i<dsi.length;){
				        ...
				        graph = new DSCapture(DSFiltergraph.RENDER_NATIVE,dsi[i][i],true,dsi[i][i], null);  
				        //graph = DSCapture.fromUserDialog(this, DSFiltergraph.VMR7,null);  
				         
				        if (controls != null) {  
				            for (int i2 = CaptureDeviceControls.BRIGHTNESS; i2 < CaptureDeviceControls.FOCUS; i2++) {  
				                try {  
				                    //     jf.add(controls.getController(i, 0, true));  
				                } catch (Exception e) {  
				                    System.out.println("Erro ao inserir controle: " + controls.getController(i2, 0, true));  
				                }  
				            }  
				        }  
...
public JPanel Cam2(int i){
		...
		if(dsi2 == null){
	        ...
	        //dsi[0][0].setPreferredFormat(1);  
	        graph2 = new DSCapture(DSFiltergraph.RENDER_NATIVE,dsi[0][i],true,dsi[0][i], null);  
	        ...
	        if (controls2 != null) {  
	            for (int i2 = CaptureDeviceControls.BRIGHTNESS; i2 < CaptureDeviceControls.FOCUS; i2++) {  
	                try {  
	                    ...
	                    System.out.println("Erro ao inserir controle: " + controls2.getController(i2, 0, true));  
	                }  
	            }  
	        }  
...
public static int[][] Cameras(DSFilterInfo[][] x){
	int Cam1, Cam2;
